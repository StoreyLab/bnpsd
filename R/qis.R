...
#' @param subpops Optional length-\eqn{k} vector of unique subpopulations in desired order.  Stops if \code{subpops} does not contain all unique labels in \code{labs} (no error if \code{subpops} contains additional labels).
#'
#' @return The \eqn{n \times k}{n-by-k} admixture proportion matrix \eqn{Q}.  The unique subpopulation labels are given in \code{colnames(Q)}.
#'
#' @examples
...
qis <- function(labs, subpops) {
    if (missing(subpops)) {
        ...
        if (!all(labs %in% subpops)) stop("Fatal: provided 'subpops' does not contain all labels in 'labs'!")
    }
