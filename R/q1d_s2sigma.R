...
##
## This function uses a numerical solver to find the value of \eqn{\sigma} (the spread of the intermediate subpopulations, more specifically the standard deviation of the normal densities that model this spread) that give the desired bias coefficient \eqn{s} given the other PSD parameters.
##
...
## sigma <- q1d_s2sigma(s=0.5, F=F, n=1000) 
q1d_s2sigma <- function(s, F, n, interval=c(0.1,10), tol=.Machine$double.eps) {
    ...
    ## q1d_sigma2s is monotonically increasing with sigma (hence extendInt='upX' gives hint of where to extend, if needed)
    ## default tolerance of .Machine$double.eps^0.25 (~ 1e-4) was kinda crummy, reduced to ~2e-16
    sigmaRoot <- stats::uniroot(function(sigma) q1d_sigma2s(sigma, F, n)-s, interval, extendInt='upX', tol=tol)
