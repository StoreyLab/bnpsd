...
coancestry_estimate <- inbr_diag(PhiHat)
...
# now plot!
x <- list(coancestry, coancestry_estimate)
titles <- c('Truth', 'Estimate')
plot_popkin(x, titles)
```
...
B <- rpint(pAnc, inbr_subpops) # draw intermediate AFs
P <- rpiaf(B, admix_proportions) # draw IAFs (individual-specific AFs)
...
Memory consumption is reduced by passing the `lowMem = TRUE` option to `rbnpsd`, which draws the genotypes `X` directly from the subpopulation AF matrix `B` and `admix_proportions`, without storing the whole IAF matrix `P` at any given time.
...
The same option exists for `rgeno`, which instead of accepting the IAF matrix `P` as input requires both `B` and `admix_proportions` as above:

```{r}
X <- rgeno(B, admix_proportions, lowMem = TRUE)
```
...
s <- 0.5 # desired bias coefficient
Fst <- 0.1 # desired FST for the admixed individuals
obj <- q1dc(n, k, s = s, F = inbr_subpops, Fst = Fst) # admixture proportions from *circular* 1D geography
admix_proportions <- obj$Q
inbr_subpops <- obj$F
...
coancestry <- coanc_admix(admix_proportions, inbr_subpops) # the coancestry matrix
# verify that we got the desired Fst!
fst(admix_proportions, inbr_subpops)
...
inbr_subpops <- inbr_subpops / popkin::fst(inbr_subpops) * Fst # normalized to have the desired Fst
popkin::fst(inbr_subpops) # verify FST for the intermediate subpopulations
...
fst(admix_proportions, inbr_subpops, weights)
```
