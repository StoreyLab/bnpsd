...
# use "cbind" to turn the vector p_anc into the column matrix "draw_genotypes_admix" expects
...
The same option exists for `draw_genotypes_admix`, which instead of accepting the IAF matrix `p_ind` as input requires both `p_subpops` and `admix_proportions` as above:
