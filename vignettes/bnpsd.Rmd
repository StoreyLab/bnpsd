...
* `draw_p_subpops` (Random $p$ for independent subpopulations)
...
P <- rpiaf(p_subpops, admix_proportions) # draw IAFs (individual-specific AFs)
...
Memory consumption is reduced by passing the `lowMem = TRUE` option to `rbnpsd`, which draws the genotypes `X` directly from the subpopulation AF matrix `p_subpops` and `admix_proportions`, without storing the whole IAF matrix `P` at any given time.
...
The same option exists for `rgeno`, which instead of accepting the IAF matrix `P` as input requires both `p_subpops` and `admix_proportions` as above:
