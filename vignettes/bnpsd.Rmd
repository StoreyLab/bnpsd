...
Memory consumption is reduced by passing the `lowMem = TRUE` option to `rbnpsd`, which draws the genotypes `X` directly from the subpopulation AF matrix `B` and the admixture coefficients `Q`, without storing the whole IAF matrix `P` at any given time.
...
w <- weights_subpops(labs) # function from `popkin` package
